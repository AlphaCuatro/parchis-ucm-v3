//  Arturo Soria y Joel Sánchez, 1ºF.
...
int primeraEn(const tFichas jugador, int casilla, tJuego juego) { // Devuelve el int correspondiente fichasEnMeta la ficha que esté en primer lugar en esa casilla
    ...
    if (a == 2) {
        ...
        for (int i = 0; i < NUM_FICHAS && primeraEn == -1; i++) {
...
int segundaEn(const tFichas jugador, int casilla) { // Devuelve el int correspondiente a la ficha que esté en segundo lugar en esa casilla
...
void cargar(tJuego& juego) { // Inicializa el archivo de guardao, si no lo encutra da el call al usuario para que lo meta en la carpeta y vuelva a intentarlo
    ...
    if (!archivo.is_open()) {
        cout << "ARCHIVO NO ENCONTRADO. COLÓQUELO EN LA CARPETA DEL JUEGO" << endl;
        goto label;
    }
...
void jugar(tJuego& juego, bool& fin) { // 
    ...
    while ((juego.tirada == 6 + siete || ficha == 0) && !fin && juego.seises != 3) {
        ...
        if (juego.tirada == 6) {
            ...
            juego.tirada = juego.tirada + siete;
        }
...
void mover(tJuego& juego, int ficha, int casilla) {
    ...
    // Despeja calle2, pasa su valor a calle1
    ...
    if (juego.jugadores[juego.turno].fichas[ficha] == zanataJugador(juego.turno)) {
        ...
        if (juego.jugadores[juego.turno].fichas[ficha] > 67 && juego.jugadores[juego.turno].fichas[ficha] < 101) {
            ...
            juego.jugadores[juego.turno].fichas[ficha] = juego.jugadores[juego.turno].fichas[ficha] - 68;
...
void setColor(tColor color) { // Función dada, relaciona el color en terminal con su tColor correspondiente
    ...
    else { cout << "No existe ninguna ficha con la que puedas contarte " << juego.premio << ".s" << endl; }
...
bool procesa5(tJuego& juego) { // Procesa una tirada de 5
    ...
    if (boolean && juego.casillas[salidaJugador(juego.turno)].calle2 != juego.turno && juego.casillas[salidaJugador(juego.turno)].calle2 != Ninguno && juego.casillas[salidaJugador(juego.turno)].calle1 == juego.turno) {
    ...
    if (boolean && juego.casillas[salidaJugador(juego.turno)].calle1 != juego.turno && juego.casillas[salidaJugador(juego.turno)].calle1 != Ninguno && juego.casillas[salidaJugador(juego.turno)].calle2 != juego.turno && juego.casillas[salidaJugador(juego.turno)].calle2 != Ninguno) {
        bool a = false;
        for (int i = 0; i < NUM_FICHAS && !a; i++) {
            if (juego.jugadores[juego.casillas[salidaJugador(juego.turno)].calle2].fichas[i] == salidaJugador(juego.turno)) {
                juego.jugadores[juego.casillas[salidaJugador(juego.turno)].calle2].fichas[i] = -1;
...
bool procesa6(tJuego& juego) { // Procesa una tirada de 6
    ...
    for (int i = 0; i < NUM_FICHAS; i++) {
        ...
        if (esSeguro(juego.jugadores[juego.turno].fichas[i])) { seguro[a] = juego.jugadores[juego.turno].fichas[i]; a++; }
    }
    ...
    if (b == 2) {
        ...
        for (int i = 0; i < NUM_FICHAS && !stop; i++) {
            ...
            if (juego.jugadores[juego.turno].fichas[i] == puente && (juego.tirada == 6 || juego.tirada == 7)) {
                ...
                casillaDestino = juego.jugadores[juego.turno].fichas[i] + juego.tirada;

                cout << "Abres un puente" << endl;
                mover(juego, i, casillaDestino);
...
bool puedeMover(const tJuego juego, int ficha, int& casilla) { // Devuleve true si todas las condiciones necesarias se cumplen

    int casillaLocal = 0;
    int premio = 0;
    int casillaDestino = casilla + juego.tirada;
    bool puedeMover = true;

    if (casillaDestino > 67 && casillaDestino < 101) { // Da la vuelta al tablero
    ...
    if (juego.jugadores[juego.turno].fichas[ficha] == zanataJugador(juego.turno)) {
        ...
        for (int i = 1; i <= juego.tirada && puedeMover == true; i++) {
    ...
    if (juego.jugadores[juego.turno].fichas[ficha] > 100) {
        ...
        for (int i = 1; i <= juego.tirada && puedeMover == true; i++) {
            ...
            casillaDestino = casilla + i;

            if (casillaDestino == 109) {
    ...
    for (int i = 1; i <= juego.tirada && puedeMover; i++) {

        int casillaDestino = casilla + i;

        if ((casillaDestino >= 68)) { casillaDestino = casillaDestino - 68; }

        if (!puente(juego, casillaDestino)) {
    ...
    if (juego.jugadores[juego.turno].fichas[ficha] != zanataJugador(juego.turno) && juego.jugadores[juego.turno].fichas[ficha] < 100) {
        ...
        for (int i = 1; i <= juego.tirada && puedeMover == true; i++) {
            ...
            if (casillaDestino == zanataJugador(juego.turno) + 1) {
    ...
    if (juego.casillas[casillaDestino].calle1 != Ninguno && juego.casillas[casillaDestino].calle2 == juego.casillas[casillaDestino].calle1 && juego.jugadores[juego.turno].fichas[ficha] < 100) { //por si en la casilla que va ficha caer ya hay dos fichas
...
bool puente(const tJuego juego, int casilla) { // Si hay dos fichas haciendo puente devuleve true
